...
Command::Command(flash_string_vector name,
    ...
    : name(std::move(name)), arguments(std::move(arguments)), execute(std::move(execute)), complete(std::move(complete)) {}
...
// 获取命令补全建议
std::vector<std::string> Command::get_completions(Shell &shell, const std::vector<std::string> &current_args, const std::string &next_arg) const {
    if (complete) {
...
// 执行指定名称的命令
void CommandRegistry::execute_command(Shell &shell, const std::string &name, const std::vector<std::string> &arguments) {
...
// 获取命令补全建议
std::vector<std::string> CommandRegistry::get_command_completions(Shell &shell, const std::string &name, const std::vector<std::string> &current_arguments, const std::string &next_argument) {
    auto it = commands.find(name);
    if (it != commands.end()) {
